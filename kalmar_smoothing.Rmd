...
# Formula: OMGE * (Y*0 - X*0) / CLIGHT - add this to rng
   ...
   to_add = rng + (OMGE * (xsat[, 1] * x[2] - xsat[, 2] * x[1]) / CLIGHT)
    ...
    # residuals = sweep(u, 1, pr - x[4])
    # residuals = residuals[ ,4]# |> as.matrix()
...
# py$least_squares(pr_residuals, x0, jac_pr_residuals, args= c(xsat_pr, pr, Wx))

count(gnss_df, utcTimeMillis)
```
...
wls = least_squares
import scipy
scipy.optimize.least_squares(r.pr_residuals, r.x0, r.jac_pr_residuals, args=(r.xsat_pr, pr, r.Wx), loss='soft_l1')
pr = r.pr
df_pr = r.df
x = np.zeros(4)
xsat_pr = np.array(r.xsat_pr)
# pr = (df_pr['SvClockBiasMeters'] - df_pr['IsrbMeters'] -           df_pr['IonosphericDelayMeters'] - df_pr['TroposphericDelayMeters']).to_numpy()
Wx = np.array(r.Wx)

scipy.optimize.least_squares(r.pr_residuals, x, r.jac_pr_residuals, args=(xsat_pr, pr, Wx), loss='soft_l1')
Wx
